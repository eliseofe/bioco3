...
public class SituatedCommModel extends AbstractCommModel<SituatedCommunicator> {
	...
	protected void receiveParcel(DefaultParcel p, long time) {
		...
		for (final SituatedCommunicator su : communicators) {
			// Release all assigned parcels in order to reallocate them. Put
			// them in unclaimedParcels
			// TODO: optimize
		...
		for (final SituatedCommunicator su : communicators) {
			...
			for (final DefaultParcel dp : unassignedParcels) {
				...
				// Get a Point which represent the communicator position. Use
				// road model got from init?
				...
				if (distance < sensingRadius) {
					...
					// communicatorPosition);
					//
					// // If distance is less than range, add dp to candidate
					// // parcels in range
					// if (distance < sensingRadius) {
					// su.addCandidateParcel(dp);
					// }
			...
			// TODO: add a step here that does something if the list of
			// candidate parcels is empty.
			// Ask Rinde what to do, it's ok a vehicle is idle right?

			// Ask su to decide the one parcel to deliver next based on the list
			// it received
			...
			if (nextParcel.isPresent()) {
				unassignedParcels.remove(nextParcel);
			}
	...
	public Set<DefaultParcel> getUnclaimedParcels() {
		return unmodifiableSet(unassignedParcels);
	}
	...
	public void claim(SituatedCommunicator claimer, DefaultParcel p) {
		checkArgument(!unassignedParcels.contains(p),
				"Parcel %s must be unclaimed.", p);
